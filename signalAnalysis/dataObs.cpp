...
RNode dataObs::run(RNode d){
  ...
  for(int i =0; i<102; i++) PDF.push_back("replica_" + std::to_string(i+1));
...
std::vector<ROOT::RDF::RResultPtr<std::vector<std::unique_ptr<TH1D>>>> dataObs::getGroupTH1(){ 
  return _h1Group;
}
std::vector<ROOT::RDF::RResultPtr<std::vector<std::unique_ptr<TH2D>>>> dataObs::getGroupTH2(){ 
  return _h2Group;
}
std::vector<ROOT::RDF::RResultPtr<std::vector<std::unique_ptr<TH3D>>>> dataObs::getGroupTH3(){ 
  return _h3Group;
}
