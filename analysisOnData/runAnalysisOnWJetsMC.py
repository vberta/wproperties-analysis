...
for region,cut in selections.iteritems():
    ...
    p.branch(nodeToStart = 'defs', nodeToEnd = 'templates_{}/Nominal'.format(rwtau), modules = [ROOT.templates(cut_wtau, weight, nom,"Nom",0)])
    p.branch(nodeToStart = 'defs', nodeToEnd = 'templates_{}/Nominal'.format(rwmu), modules = [ROOT.templates(cut_wmu, weight, nom,"Nom",0)])            
    ...
    for s,variations in systematics.iteritems():
        ...
        if not runBKG: 
            p.branch(nodeToStart = 'defs'.format(region), nodeToEnd = 'prefit_{}/{}Vars'.format(rwtau,s), modules = [ROOT.muonHistos(cut_wtau, weight,vars_vec,variations[1], 0)])
            p.branch(nodeToStart = 'defs'.format(region), nodeToEnd = 'prefit_{}/{}Vars'.format(rwmu,s), modules = [ROOT.muonHistos(cut_wmu, weight,vars_vec,variations[1], 0)])
                
        p.branch(nodeToStart = 'defs'.format(region), nodeToEnd = 'templates_{}/{}Vars'.format(rwtau,s), modules = [ROOT.templates(cut_wtau, weight,vars_vec,variations[1], 0)])
        p.branch(nodeToStart = 'defs'.format(region), nodeToEnd = 'templates_{}/{}Vars'.format(rwmu,s), modules = [ROOT.templates(cut_wmu, weight,vars_vec,variations[1], 0)])
                    

    #column variations#weight will be nominal, cut will vary
    for vartype, vardict in selectionVars.iteritems():
        cut_vec_wtau = ROOT.vector('string')()
        cut_vec_wmu = ROOT.vector('string')()
        var_vec = ROOT.vector('string')()
        for selvar, hcat in vardict.iteritems() :
            newcut_wmu = cut_wmu.replace('MT', 'MT_'+selvar)
            newcut_wtau = cut_wtau.replace('MT', 'MT_'+selvar)
        
            if 'corrected' in selvar:
                newcut_wmu = newcut_wmu.replace('Mu1_pt', 'Mu1_pt_'+selvar)
                newcut_wtau = newcut_wtau.replace('Mu1_pt', 'Mu1_pt_'+selvar)

            cut_vec_wtau.push_back(newcut_wtau)
            cut_vec_wmu.push_back(newcut_wmu)
            var_vec.push_back(selvar)
        print "branching column variations:", vartype, "\tvariations:", var_vec
        if not runBKG: 
            p.branch(nodeToStart = 'defs', nodeToEnd = 'prefit_{}/{}Vars'.format(rwtau,vartype), modules = [ROOT.muonHistos(cut_vec_wtau, weight, nom,"Nom",hcat,var_vec)])  
            p.branch(nodeToStart = 'defs', nodeToEnd = 'prefit_{}/{}Vars'.format(rwmu,vartype), modules = [ROOT.muonHistos(cut_vec_wmu, weight, nom,"Nom",hcat,var_vec)])  
            
        p.branch(nodeToStart = 'defs', nodeToEnd = 'templates_{}/{}Vars'.format(rwtau,vartype), modules = [ROOT.templates(cut_vec_wtau, weight, nom,"Nom",hcat,var_vec)])  
        p.branch(nodeToStart = 'defs', nodeToEnd = 'templates_{}/{}Vars'.format(rwmu,vartype), modules = [ROOT.templates(cut_vec_wmu, weight, nom,"Nom",hcat,var_vec)])  
