{
  ...
  TFile* _file_trigger_plus = TFile::Open("data/externalSF/fromWhelicity/smoothEfficiency_muons_recoToSel_finerETA.root");

  std::map<std::string, TH3D*> cmap;
  std::map<std::string, TH2D*> vmap;
  std::map<std::string, TH2D*> hmap;
  hmap.insert( std::pair<std::string, TH2D*>("reco",  (TH2D*)_file_trigger_plus->Get("scaleFactor_etaInterpolated") ) );
  cmap.insert( std::pair<std::string, TH3D*>("cov_mc",   (TH3D*)_file_trigger_plus->Get("hist_ErfCovMatrix_vs_eta_mc")));
  vmap.insert( std::pair<std::string, TH2D*>("val_mc",   (TH2D*)_file_trigger_plus->Get("hist_ErfParam_vs_eta_mc")));
  std::vector<std::string> data = {"mc"};
  std::vector<std::string> charges = {"plus"};
  std::vector<std::string> vars = {"Up","Down"};
  for(auto d : data){
    for(auto c : charges){
      TH3D* cov = cmap.at("cov_"+d);
      TH2D* val = vmap.at("val_"+d);
      for(int e = 0 ; e<3 ; e++ ){
	for(auto v : vars ){
	  TH2D* h = (TH2D*)hmap.at("reco")->Clone(("reco_"+d+"_eigen"+std::to_string(e)+v).c_str());
	  h->Reset();
	  hmap.insert( std::pair<std::string, TH2D*>("reco_"+d+"_eigen"+std::to_string(e)+v, h) );
	}
      }
	...
	TH2D* icov = (TH2D*)((TH2D*)cov->Project3D("yz"))->Clone((d+"_"+std::to_string(ieta)).c_str());
	...
	for(int e = 0 ; e<3 ; e++ ){
	  for(auto v : vars ){
	    ...
	    TH2D* h = hmap.at("reco_"+d+"_eigen"+std::to_string(e)+v);
	    for(int ipt=1; ipt<=h->GetYaxis()->GetNbins(); ipt++){
	      ...
	      std::cout << "Eig[" << e << "] "+v+" at (" << eta << "," << pt << ") : " << setprecision(5) <<  "," << h->GetBinContent(ieta,ipt) <<  std::endl;	      	      
	    }
	  }
	}
