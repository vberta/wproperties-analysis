...
public:
    Replica2Hessian() : ptArr_(nBinsPt_ + 1)
    {
        ...
        for (unsigned int i = 0; i < nPdfWeights_; i++) PDFArrRepl_.push_back("replica_"+std::to_string(i));
        for (unsigned int i = 0; i < nPdfEigWeights_; i++) PDFArrHess_.push_back("hess_" + std::to_string(i));
        };
        ~Replica2Hessian(){};
