#ifndef SELECTIONDEFS_H
#define SELECTIONDEFS_H
...
//A class which simply performs a Filter on a node
class selectionDefs : public Module
{
...
public:
    selectionDefs(std::string filter){
      _filter = filter;
    };
    ~selectionDefs(){};

    RNode run(RNode) final;
