...
for region,cut in selections.iteritems():
    ...
    p.branch(nodeToStart = 'prefit_{}'.format(region), nodeToEnd = 'prefit_{}/Nominal'.format(region), modules = [ROOT.muonHistos(cut, weight, nom,"Nom",0)])    
    #weight variations
    for s,variations in systematics.iteritems():
        ...
        p.branch(nodeToStart = 'prefit_{}'.format(region), nodeToEnd = 'prefit_{}/{}Vars'.format(region,s), modules = [ROOT.muonHistos(cut, weight,vars_vec,variations[1], 0)])
    #column variations#weight will be nominal, cut will vary
    for selvar, hcat in selectionVars.iteritems():
        newcut = cut.replace('MT', 'MT_'+selvar)
        if 'corrected' in selvar:
            newcut = newcut.replace('Mu1_pt', 'Mu1_pt_'+selvar)
        print newcut, '\t', hcat
        p.branch(nodeToStart = 'defs', nodeToEnd = 'prefit_{}/{}'.format(region, selvar), modules = [ROOT.muonHistos(newcut, weight, nom,"Nom",hcat,selvar)])  
