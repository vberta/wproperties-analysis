...
RNode rochesterVariations::run(RNode d)
{
  ...
  for(unsigned int idx = 0; idx < 99; idx++) {
    ...
    std::cout << "Upnames-" << ptUp << "\t" << mtUp << std::endl; 
    std::cout << "Downames-" << ptDown << "\t" << mtDown << std::endl;
 
    df = df.Define(ptDown, [&idx,this](float pt, float eta) { return getCorrfromhisto(_statEigenminusv, pt, eta, idx);}, 
		   {"Mu1_pt", "Mu1_eta"})
      .Define(mtDown, W_mt, {ptDown, "Mu1_phi", "MET_pt_nom", "MET_phi_nom"})
      .Define(ptUp, [&idx,this](float pt, float eta) { return getCorrfromhisto(_statEigenplusv, pt, eta, idx);}, 
		  {"Mu1_pt", "Mu1_eta"})
