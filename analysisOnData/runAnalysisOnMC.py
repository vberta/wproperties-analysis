...
def RDFprocessMC(fvec, outputDir, sample, xsec, fileSF, systType, pretendJob=True,SBana=False):
    ...
    for region,cut in list(selections_bkg.items()):
       ...
       #weight variations
        for s,variations in list(systematicsFinal.items()):
...
def main():
    ...
    for sample in samples:
        ...
        RDFtrees[sample] = RDFprocessMC(fvec, outputDir, sample, xsec, fileSF, systType, pretendJob, SBana)

    #now trigger all the event loops at the same time:
    objList = []
    for sample in samples:
        if not samples[sample]['datatype'] == 'MC':
            continue
        if 'WJets' in sample:
            continue
